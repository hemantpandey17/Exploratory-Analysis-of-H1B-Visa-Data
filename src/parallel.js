function parallel(filename){
	...
	d3.csv(filename, function(error, cars) {
	  ...
	  x.domain(dimensions = d3.keys(cars[0]).filter(function(d) {
	    return ((d == 'PREVAILING_WAGE') && (y[d] = d3.scale.linear()
	        .domain(d3.extent(cars, function(p) { return +p[d]; }))
	...
	function brush() {
	  var actives = dimensions.filter(function(p) { return !y[p].brush.empty(); }),
	  		// extents = d3.map(actives, function(p){return y[p].brush;}).keys();
	  ...
	  foreground.style("display", function(d) {
	    ...
	    return actives.every(function(p, i) {
	    	...
	    	out = ((extents[i][0] <= d[p] && d[p] <= extents[i][1]) || (extents[i][0] <= y[p](d[p]) && y[p](d[p]) <= extents[i][1]));
	    	// console.log(out);
	      return out;
	    }) ? null : "none";
	  });
